...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-slate-950/80 backdrop-blur-md border-b border-slate-800 text-white z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="text-xl font-bold bg-linear-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent cursor-pointer">
          <Link to="accueil" smooth={true} duration={500} offset={-70} onClick={closeMenu}>
            Portfolio
          </Link>
        </div>
          ...
          {navLinks.map((link, index) => (
            <li key={index}>
              <Link
                ...
                duration={500}
                spy={true}
                offset={-70} // Compense la hauteur de la Navbar fixe
